define(
	['app/utils/class', 'app/core/errors/base-error'],
	function (ClassUtils, BaseError) {
		'use strict';

		var errorCodes = BaseError.Codes,
			genericErrorTitle = 'Aw, snap!',
			genericErrorMessage = 'Something has gone wrong with your ' +
				'request. Please try again in a few seconds.',
			apiErrorCodes = {
		...
		return ClassUtils.define(function(config, helpers, state){
			...
			_handleAPIErrors: function(error){
				...
				if (errorCodes.isSignUpError(error.originalError.code)) {
					isHandled = true;
					this._helpers.win.showPrompt('Unable to Sign Up',
						'There was a problem trying to Sign Up. ' +
							'Please try again later.'
					);
				} else if (errorCodes.isSignInError(error.originalError.code)) {
					isHandled = true;
					this._helpers.win.showPrompt('Unable to Sign In',
			...
			handle: function(e){
				...
				if(BaseError.isCanceled(error)){
					...
					// TEMP DIRTY HACK, to avoid List\Flipview layout exceptions
					// on fast navigation between pages
					isHandled = true;
				}

				if (!isHandled) {

					// This is just for development mode, we don't crash app but
					// show error prompt
			...
			getDetailedUnhandledErrorMessage: function(e){
				...
				if(this.isDevelopmentMode()){
					if(typeof e === 'string'){
						...
						message = (
							BaseError.isBaseError(e) && e.originalError ?
								e.originalError.message ||
									e.originalError.description
								: null
